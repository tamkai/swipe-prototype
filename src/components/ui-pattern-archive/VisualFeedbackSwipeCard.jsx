...
const VisualFeedbackSwipeCard = ({ question, onSwipe }) => {
  ...
  const handleDragEnd = async (event, info) => {
    ...
    if (Math.abs(info.offset.x) > threshold) {
      // スワイプ判定（左スワイプ=YES、右スワイプ=NO）
      const direction = info.offset.x > 0 ? 'no' : 'yes';
      ...
      // 枠線の色を設定
      setBorderColor(direction === 'yes' ? '#10b981' : '#ef4444');
...
export default VisualFeedbackSwipeCard;
